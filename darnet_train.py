...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='E:\PycharmProject\BiFA\config\whudarnet.json',
    ...
    elif opt['model']['loss'] == 'hybrid_loss':
    ...
    if opt['phase'] == 'train':
        ...
        for current_epoch in range(0, opt['train']['n_epoch']):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                preds = cd_model(train_im1, train_im2)
                pred_img = preds[-1]
                gt = train_data['L'].to(device).long()
                train_loss = loss_fun(preds, gt)
                optimer.zero_grad()
                train_loss.backward()
                optimer.step()
            ...
            with torch.no_grad():
                if current_epoch % opt['train']['val_freq'] == 0:
                    ...
                    for current_step, val_data in enumerate(val_loader):
                        ...
                        preds = cd_model(val_img1, val_img2)
                        pred_img = preds[-1]
                        gt = val_data['L'].to(device).long()
                        val_loss = loss_fun(preds, gt)
        ...
        if load_path is not None:
            ...
            with torch.no_grad():
                for current_step, test_data in enumerate(test_loader):
                    ...
                    preds = cd_model(test_img1, test_img2)
                    pred_img = preds[-1]
