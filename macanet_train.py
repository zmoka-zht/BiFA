...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='E:\PycharmProject\BiFA\config\whumscanet.json',
    ...
    if opt['phase'] == 'train':
        ...
        for current_epoch in range(0, opt['train']['n_epoch']):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                pred_img1, pred_img2, pred_img3 = cd_model(train_im1, train_im2)
                gt = train_data['L'].to(device).long()
                train_loss = loss_fun(pred_img1, gt) + loss_fun(pred_img2, gt) + loss_fun(pred_img3, gt)
                optimer.zero_grad()
                train_loss.backward()
                optimer.step()
                ...
                if current_step % opt['train']['train_print_iter'] == 0:
                    ...
                    out_dict['pred_cm'] = torch.argmax(pred_img1, dim=1, keepdim=False)
                    out_dict['gt_cm'] = gt
                    visuals = out_dict
            ...
            with torch.no_grad():
                if current_epoch % opt['train']['val_freq'] == 0:
                    ...
                    for current_step, val_data in enumerate(val_loader):
                        ...
                        pred_img1, pred_img2, pred_img3 = cd_model(val_img1, val_img2)
                        gt = val_data['L'].to(device).long()
                        val_loss = loss_fun(pred_img1, gt) + loss_fun(pred_img2, gt) + loss_fun(pred_img3, gt)
        ...
        if load_path is not None:
            ...
            with torch.no_grad():
                for current_step, test_data in enumerate(test_loader):
                    ...
                    pred_img1, pred_img2, pred_img3 = cd_model(test_img1, test_img2)
