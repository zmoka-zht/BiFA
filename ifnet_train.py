...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='E:\PycharmProject\BiFA\config\whuifnet.json',
    ...
    if opt['phase'] == 'train':
        ...
        for current_epoch in range(0, opt['train']['n_epoch']):
            ...
            for current_step, train_data in enumerate(train_loader):
                ...
                out5, out4, out3, out2, out1, gt5, gt4, gt3, gt2, gt1 = cd_model(train_im1, train_im2, gt)
                pred_img = out5
                train_loss = loss_fun(out5, gt5) + loss_fun(out4, gt4) + loss_fun(out3, gt3) + loss_fun(out2, gt2) + loss_fun(out1, gt1)
                optimer.zero_grad()
                train_loss.backward()
                optimer.step()
            ...
            with torch.no_grad():
                if current_epoch % opt['train']['val_freq'] == 0:
                    ...
                    for current_step, val_data in enumerate(val_loader):
                        ...
                        #pred_img = cd_model(val_img1, val_img2, gt)
                        gt = val_data['L'].to(device)#.long()
                        out5, out4, out3, out2, out1, gt5, gt4, gt3, gt2, gt1 = cd_model(val_img1, val_img2, gt)
                        pred_img = out5
                        val_loss = loss_fun(out5, gt5) + loss_fun(out4, gt4) + loss_fun(out3, gt3) + loss_fun(out2, gt2) + loss_fun(out1, gt1)
        ...
        if load_path is not None:
            ...
            with torch.no_grad():
                for current_step, test_data in enumerate(test_loader):
                    ...
                    out5, out4, out3, out2, out1, gt5, gt4, gt3, gt2, gt1 = cd_model(test_img1, test_img2, gt)
                    pred_img = out5
